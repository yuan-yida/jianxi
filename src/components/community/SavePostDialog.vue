...
const props = defineProps<{ postId: string; show: boolean }>();
const emit = defineEmits<{ (e: 'update:show', v: boolean): void }>();

const { t } = useI18n();
const community = useCommunityStore();

const post = computed(() => community.getPost(props.postId));
...
watch(
  ...
  (open) => {
    if (!open) return;
    const rec = community.getSavedRecord(props.postId);
    selected.value = rec ? [...rec.collectionIds] : [];
    showCreate.value = false;
  },
);

const isSaved = computed(() => community.isSaved(props.postId));
...
const confirmSave = () => {
  community.savePost(props.postId, selected.value);
  close();
};

const removeSave = () => {
  community.unsavePost(props.postId);
  close();
};
    ...
    >
      ...
      <div class="relative w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl p-5 shadow-xl max-h-[85vh] overflow-y-auto">
        ...
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-base font-bold text-slate-800">🔖 {{ t('community.favorites.saveTitle') }}</h3>
        ...
        <div class="flex items-center gap-3 bg-slate-50 rounded-xl p-3 mb-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-xs font-semibold text-slate-700">{{ post.author.name }}</div>
            <p class="text-xs text-slate-400 truncate">{{ post.content }}</p>
          </div>
          ...
          <div class="space-y-1.5">
            ...
            >
              ...
              <span class="ml-auto text-[11px] text-slate-400">
                {{ community.savedPostViews.filter((v) => v.collectionIds.length === 0).length }}
              </span>
            </button>
            <button
            ...
            >
              ...
              <span>{{ c.name }}</span>
              <span class="ml-auto text-[11px] opacity-70">{{ community.savedPostsOfCollection(c.id).length }}</span>
              <span
